...
if (isset($_POST['submit'])) 
{
   ...
   if(isset($_POST['hidden-total-price']))
   {
    ...
    $insertQuery ="INSERT INTO subscription_order(Name, Address,Phone,SelectedItem,Weight,TotalPrice,PickupDate,PickupTime,DeliveryDate,DeliveryTime) VALUES('$name','$address','$phone','$selectedItem','$weight','$totalPrice','$pickupdate','$pickuptime','$deliverydate','$deliverytime')";
            ...
            <table>
                ...
                <tbody>
                  ...
                  // Include your database connection
                    $serviceQuery = "SELECT * FROM subscription";
                    $serviceResult = mysqli_query($conn, $serviceQuery);

                    while ($row = mysqli_fetch_assoc($serviceResult)) {
                        echo "<tr>";
                        echo "<td>{$row['package_name']}</td>";
                        echo "<td>Rs {$row['package_price']}</td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
        ...
        <div class="forms-container">
            <form class="pick" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" novalidate>
                ...
                <select name="item" id="item" onchange="updatePriceAndTotal()">
                    ...
                    <?php
                    $serviceDat = array(); // Store service data for JavaScript
                    $serviceQuer = "SELECT * FROM subscription ";
                    $serviceResul = mysqli_query($conn, $serviceQuer);

                    while ($row = mysqli_fetch_assoc($serviceResul)) {
                        $serviceDat[] = $row;
                        echo "<option value='{$row['package_name']}'>{$row['package_price']}</option>";
                    }
                    ?>
                </select>
                ...
                <script>
                    ...
                    const services = <?php echo json_encode($serviceDat); ?>;
                    ...
                    function populateServiceOptions() {
                        ...
                        services.forEach(service => {
                            ...
                            option.value = service.package_price;
                            option.text = service.package_name;
                    ...
                    function updatePriceAndTotal() {
                        ...
                        const selectedService = services.find(service => service.package_price === selectedItem.value);

                        if (selectedService) {
                            const pricePerUnit = parseFloat(selectedService.package_price);
