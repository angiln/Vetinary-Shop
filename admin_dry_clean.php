...
//    }
include  'db.php';
$sql = "SELECT * FROM dry_cleaning";
$result = $conn -> query ($sql);
if(isset($_POST['update_update_btn'])){
    $update_value = $_POST['update_status'];
    $update_id = $_POST['update_id'];
    $update_quantity_query = mysqli_query($conn, "UPDATE `dry_cleaning` SET status = '$update_value' WHERE id = '$update_id'");
    if($update_quantity_query){
       header('location:admin_dry_clean.php');
  ...
  if(isset($_GET['remove'])){
    ...
    mysqli_query($conn, "DELETE FROM `dry_cleaning` WHERE id = '$remove_id'");
    header('location:admin_dry_clean.php');
  };
...
<div class="container">
  <h1 class = "heed" >Dry Clean Order</h1>
<table class="table">
  <thead>
    <tr>
    <th scope="col">S.N</th>
            ...
            while($row = mysqli_fetch_assoc($result)) 
            {
              ...
              ?>
                ...
                <td><?php if( $row["SelectedItem"]==120){
                          echo "Shirt/T-Shirt";
                          } elseif($row["SelectedItem"]==125){ echo "Pant/Trouser";}
                          else { echo "Coat ";}
                 ?></td>
                ...
                <!-- <td><a href="admin_order.php?remove=<?php echo $row['id']; ?>">remove</a></td> -->
                <td><a href="admin_dry_clean.php?remove=<?php echo $row['id']; ?>"><img style="width:25px; height:25px;"  src="del.png"/></a></td>
