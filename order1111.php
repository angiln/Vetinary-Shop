...
if (isset($_POST['submit'])) {
    // Get form data
    $name = $_POST['fullname']; // Change 'fullname' to 'name'
    $phone = $_POST['phone'];
    $address = $_POST['address'];
    $email = $_POST['email']; // If you have an email field in your form
    $selectedItem = $_POST['item']; // If you have this field
    $weight = $_POST['weight'];
    $totalPrice = $_POST['hidden-total-price'];
    $pickupdate = $_POST['pickup-date']; // Change 'pickupDate' to 'pickupdate'
    $pickuptime = $_POST['pickup-time'];
    $deliverydate = $_POST['delivery-date']; // Change 'deliveryDate' to 'deliverydate'
    $deliverytime = $_POST['delivery-time'];

    $insertQuery = "INSERT INTO order_new (Name, Address, Phone, PickupDate, PickupTime, DeliveryDate, DeliveryTime)
    VALUES ('$name', '$address', '$phone', '$pickupdate', '$pickuptime', '$deliverydate', '$deliverytime')";
...
if ($result) {
    // Data added successfully, you can redirect to a success page or perform other actions
...
<head>
    ...
    <style>
        ...
        .right-side h2
        {
            ...
            font-weight: lighter;
            font-family: 'arial', 'calibri', 'sans-serif';
            /*schedule and pickup delivery form *
            /* margin-bottom: -7%;
            margin-top:2% ; */
            margin: 5%;
